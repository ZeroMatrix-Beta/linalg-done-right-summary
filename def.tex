...
% BEGIN AI notes ********************************************
% A visually distinct box for remarks added during the AI audit:
% sanity checks, alternative routes, worked micro-examples,
% mnemonics and warnings about common traps.
% Usage:  \begin{ainote} ... \end{ainote}
%         \begin{ainote}[Sanity check] ... \end{ainote}
\definecolor{ainoterule}{RGB}{70,95,130}
\definecolor{ainotebg}{RGB}{244,247,251}
\definecolor{ainotetitle}{RGB}{40,60,90}
...
\newtcolorbox{ainote}[1][AI Note]{%
  ...
  after skip  = 0.45em,
  fontupper = \small,
  coltitle  = ainotetitle,
  attach title to upper = {\hspace{0.4em}},
  title     = {\prooffont{#1:}},
}

% Inline variant, for a one-clause aside that does not deserve a box.
\newcommand{\ainoteinline}[1]{{\small\color{ainotetitle}\prooffont{AI Note:} #1}}

% A proof attempt that does not work, kept on purpose so the mistake stays
% visible and instructive. Always follow it with an \ainote that names the
% exact step which breaks, then give the correct argument.
% Usage:  \begin{failedattempt} ... \end{failedattempt}
%         \begin{failedattempt}[Second attempt (also fails)] ... \end{failedattempt}
\definecolor{failrule}{RGB}{160,110,105}
\definecolor{failbg}{RGB}{251,248,247}
\definecolor{failtitle}{RGB}{120,70,65}

\newtcolorbox{failedattempt}[1][First attempt (does not work)]{%
  enhanced jigsaw,
  breakable,
  colback   = failbg,
  colframe  = failrule,
  ...
  attach title to upper = {\hspace{0.4em}},
  title     = {\prooffont{#1:}},
  borderline west = {1.2pt}{0pt}{failrule, dashed},
}
% END AI notes **********************************************
