...
\begin{prf} We need to show the proposition in two directions.
  \begin{description}
    ...
    \item{\Leftarrowdirection} The first condition ensures that the additive identity $0$ of $V$ is in $U$. The other two conditions make sure that addition and scalar multiplication make sense on $U$.
...
\begin{prf}
  ...
  Conversely, every subspace of $V$ containing $V_1, \ldots, V_m$ contains $W$, because such a subspace is closed under addition. Thus, $W$ is the smallest subspace of $V$ containing $V_1, \ldots, V_m$.
\end{prf}
...
\begin{prf}
    ...
    Because $0 \in V_k$ for every $\kInOneTillM$, one way of writing $0$ as such a sum is $0 = 0 + \cdots + 0$.
    By uniqueness of decompositions, that must be the \emph{only} way, and so the only way to write $0 = v_1 + \cdots + v_m$ is $v_1 = \cdots = v_m = 0$.
    ...
    Subtracting the second decomposition from the first gives
    \begin{equation}
      0= (v_1-u_1)+\cdots+(v_m-u_m).
    \end{equation}

    Each $V_k$ is a subspace, hence closed under addition and under additive inverses, so $v_k - u_k \in V_k$ for every $\kInOneTillM$. The equation above is therefore genuinely a way of writing $0$ as a sum $w_1 + \cdots + w_m$ with $w_k := v_k - u_k \in V_k$, which is exactly the situation the hypothesis speaks about.
...
\begin{prf} Let $U$ and $W$ be subspaces of the same vector space.
  \begin{description}
    ...
    \begin{equation}
      ...
      0 = u+w.
    \end{equation}
    By \ref{thm: condition for a direct sum}, we know that our goal is to show that this implies $u=w=0$. The equation \eqref{eq: zero equals u plus w} above implies that $u=-w\in W$ and $w=-u \in U$, because $U$ and $W$ are closed under additive inverses. Thus, $u,w \in U\cap W$. Hence, $u=w=0$. Therefore, $U+W$ is a direct sum.
  \end{description}
  \vspace{-\baselineskip}
\end{prf}

\begin{ainote}[Trap]
  \ref{thm: sum and intersection of two subspaces} is a statement about \emph{two} subspaces only. For $m \geq 3$ it is \emph{not} enough that the $V_k$ intersect pairwise in $\{0\}$. Take $V = \myF^2$ and
  \begin{equation}
    V_1 = \set{(x,0) \mid x \in \myF}, \quad
    V_2 = \set{(0,y) \mid y \in \myF}, \quad
    V_3 = \set{(z,z) \mid z \in \myF}.
  \end{equation}
  Any two of them meet only in $0$: a vector in $V_1 \cap V_3$ has second coordinate $0$ and two equal coordinates, hence is $0$, and similarly for the other two pairs. Yet the sum is \emph{not} direct, because
  \begin{equation}
    0 = \underbrace{(1,0)}_{\in \, V_1} + \underbrace{(0,1)}_{\in \, V_2} + \underbrace{(-1,-1)}_{\in \, V_3}
  \end{equation}
  writes $0$ in a way where not every summand is $0$. So for general $m$ the criterion to reach for is \ref{thm: condition for a direct sum}, never pairwise intersections. A pairwise-looking condition that \emph{does} work is
  \begin{equation}
    \textstyle V_k \cap \sum_{j \neq k} V_j = \{0\} \quad \forall \kInOneTillM,
  \end{equation}
  where the intersection is taken against the sum of \emph{all the others} at once, not against one other at a time. In the example above this fails already for $k=1$, since $V_2 + V_3 = \myF^2 \supseteq V_1$.
\end{ainote}
