...
\begin{mydef}[matrix of a linear map, $\mmatrix(T)$]
  ...
  Let $T \in \linmap (V,W)$, let $v_1, \dots, v_n$ be a basis of $V$, and let $w_1, \dots, w_m$ be a basis of $W$. The \qt{matrix of $T$} with respect to these bases is the $m$-by-$n$ matrix $\mmatrix(T)$ whose entries $A_{j,k}$ are defined by
  \begin{equation}
    T v_k = A_{1,k} w_1 + \cdots + A_{m,k} w_m = \sum_{j=1}^{m} A_{j,k} w_j.
  \end{equation}
  If the bases are not clear from the context, one writes $\mmatrix(T, (v_1, \dots, v_n),(w_1, \dots, w_m))$. One remembers how to construct $\mmatrix(T)$ if one writes the bases on the left-hand side and on top of the matrix. It looks as follows:
  \begin{equation}
    \mathcal{M} (T) :\equiv
...
\begin{thm}[matrix of a scalar times a linear map]
  \label{thm: matrix of a scalar times a linear map}
...
For now, let $A := \mmatrix(S)\in \myF^{m,n}$ and $B := \mmatrix(T)\in \myF^{n,p}$. The matrices of $S$ and $T$ look as follows:
\begin{equation}
  A := \mathcal{M} (S) =
  \begin{blockarray}{cccccc}
...
\begin{mydef} [matrix multiplication]
  ...
  Let $A \in \myF^{m,n}$ and $B \in \myF^{n,p}$. Then $AB \in \myF^{m,p}$ is the matrix whose entry in row $j$, column $k$, is given by the equation
  \begin{equation}
...
\end{thm}
\begin{prf}
  See the derivation at the beginning of this subsection.
\end{prf}
...
\begin{mydef} [notation: $A_{j, \mathsmaller{\bullet}}, A_{\mathsmaller{\bullet}, k}$ ] Let $A\in \myF^{m,n}$. Then:
  \begin{itemize}
    ...
    \item If $1 \leq k \leq n$, then $A_{\mathsmaller{\bullet}, k}$ denotes the $m$-by-$1$ matrix consisting of column $k$ of $A$.
  \end{itemize}
\end{mydef}
...
\begin{mydef}[transpose, $A^t$]
  ...
  \begin{equation}
    \left( A^{t} \right)_{k,j} = A_{j,k} \quad \forall k \in \{1, \ldots, n\}, j \in \{1, \ldots, m\}.
  \end{equation}
\end{mydef}
...
\begin{thm}[column-row factorization]
  \label{thm: column-row factorization}
  Suppose $A \in \myF^{m,n}$ with column rank $c \geq 1$. Then there exist $C\in \myF^{m,c}$ and $R\in \myF^{c,n}$ such that
  \begin{equation}
    A = CR.
  \end{equation}
\end{thm}
\begin{prf}
  Let $c$ be the column rank of $A$. Then the subspace of $\myF^{m,1}$ spanned by the columns of $A$ has dimension $c$. Let $C_{\mathsmaller{\bullet}, 1}, \ldots, C_{\mathsmaller{\bullet}, c} \in \myF^{m,1}$ be a basis of this subspace, and let $C \in \myF^{m,c}$ be the matrix whose columns are these basis vectors.
  Since each column $A_{\mathsmaller{\bullet}, k}$ of $A$ is in the span of the columns of $C$, for each $k \in \{1, \ldots, n\}$ there exist scalars $R_{1,k}, \ldots, R_{c,k} \in \myF$ such that
  \begin{equation}
    A_{\mathsmaller{\bullet}, k} = R_{1,k} C_{\mathsmaller{\bullet}, 1} + \cdots + R_{c,k} C_{\mathsmaller{\bullet}, c}.
  \end{equation}
  Let $R \in \myF^{c,n}$ be the matrix whose entry in row $j$, column $k$ is $R_{j,k}$. By \ref{thm: linear combination of columns}, column $k$ of $CR$ equals $A_{\mathsmaller{\bullet}, k}$. Hence, $A = CR$.
\end{prf}

 % 3.57
\begin{thm}[column rank and row rank]
  \label{thm: column rank equals row rank}
  If $A\in \myF^{m,n}$, then
  \begin{equation}
    \text{column rank of } A = \text{row rank of } A.
  \end{equation}
\end{thm}
\begin{prf}
  If $A = 0$, both ranks equal $0$. If $A \neq 0$, let $c \ge 1$ be the column rank of $A$.
  By \ref{thm: column-row factorization}, there exist $C \in \myF^{m,c}$ and $R \in \myF^{c,n}$ such that $A = CR$.
  By row-wise matrix multiplication, each row of $A$ is a linear combination of the $c$ rows of $R$.
  Therefore, the span of the rows of $A$ is contained in the span of the $c$ rows of $R$, which implies
  \begin{equation}
    \text{row rank of } A \leq c = \text{column rank of } A.
  \end{equation}
  Applying this inequality to $A^t$, we get
  \begin{equation}
    \text{column rank of } A = \text{row rank of } A^t \leq \text{column rank of } A^t = \text{row rank of } A.
  \end{equation}
  Combining both inequalities yields $\text{column rank of } A = \text{row rank of } A$.
\end{prf}
