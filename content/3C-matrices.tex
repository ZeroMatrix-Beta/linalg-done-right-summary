...
\end{thm}
\begin{prf}
  It is easy to verify that $\myF^{m,n}$ is a vector space. The list of length $mn$ that consists of distinct $m$-by-$n$ matrices that have $0$ in all entries exept a $1$ in one entry is a basis of $\myF^{m,n}$. Hence,
  $\dim \myF^{m,n}=mn$.
\end{prf}
