...
\begin{mydef} [span of a list of vectors]
  ...
  The definition also implies that $\myspan{ 0} = \{0\}$, or written differently, $\myspan{\vec 0} = \{\vec 0\}$ . Note that sometimes we write $\vec 0$ and somtetimes we write $0$ throughout this document. Later on, $0$ sometimes also means the zero-operator.
\end{mydef}
