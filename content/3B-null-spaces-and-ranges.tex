...
\begin{prf} We want to know whether a system of linear equations has no solutions for some choice of constant terms on the right-hand side. Fix positive integers $m$ and $n$, and let $A_{j,k} \in \myF$ for $j = 1, \ddd, m$ and $k = 1, \ddd, n$. For $c_1, \ddd, c_m \in \myF$, consider:
  ...
  We want to know whether $\myrange T = \myF^m$ or $\myrange T \neq \myF^m$. What condition ensures that $T$ is not surjective? By \ref{thm: linear map to a higher-dimensional space is not surjective}, this holds if $\dim \myF^n < \dim \myF^m$ (i.e., if $n<m$). This means that if there are more equations than variables in a system of linear equations, then there are no solutions for some constant terms.
\end{prf}
