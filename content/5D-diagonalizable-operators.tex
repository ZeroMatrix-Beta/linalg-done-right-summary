...
\begin{prf} For this proof, let $T\in \linmap (V)$ be finite-dimensional ($\dim V \neq \infty$).
  ...
  \Rightarrowdirection Suppose $T$ is diagonalizable. Thus, there exists a basis $v_1, \ddd, v_n$ of $V$ consisting of eigenvectors of $T$.  Let $\lambda_1, \ddd, \lambda_m$ be said distinct eigenvalues of $T$. Therefore, for every $v_j$ we have  $(T-\lambda_k I)v_j = 0.$
  Thus,
  \begin{equation}
...
%  \end{equation}
  ...
  Now, suppose $m>1$ that and the desired result holds for all values smaller than $m$. Like in the proof of theorem \ref{thm: necessary and sufficient condition to have an upper-triangular-matrix}, we observe that $\myrange (T-\lambda_m I)$ is invariant under any polynomial of $T$ (using \ref{thm: null space and range of p(T) are invariant under T} with $p(z) := z-\lambda_m$). Now, define
  \begin{equation}
