...
\begin{thm} [conditions equivalent to diagonalizability]
  ...
  Suppose $V$ is finite-dimensional and let $\lambda_1, \dots,\lambda_m$ denote the distinct (i.e., $\lambda_j \neq \lambda_k$ for $j \neq k$) eigenvalues of $T\in \linmap (V)$. Then the following are equivalent:
...
\begin{prf} We prove (a) $\iff$ (b), (b) $\implies$ (c), (c) $\implies$ (d), and then (d) $\implies$ (b). Let $\lambda_1, \ddd, \lambda_m$ be distinct eigenvalues of $T \in \linmap (V)$ for the whole proof.
  ...
  with respect to a basis $v_1, \ddd, v_n$ of $V$, where $n = \dim V$ and the diagonal entries $\alpha_1, \ddd, \alpha_n$ are eigenvalues of $T$, listed with repetitions. This is the case $\iff Tv_k = \alpha_k v_k$ for $k = 1, \ddd, n$, that is, $\iff$ each $v_k$ is an eigenvector of $T$. Thus, (a) and (b) are equivalent, i.e., (a) $\iff$ (b).
  ...
  \StepThree (c) $\implies$ (d) follows immediately from \ref{thm: a sum is a direct sum if and only if the dimensions add up}, which states that the dimensions of a direct sum add up $\left(\text{i.e., } \dim \left(\bigoplus_{j=1}^m E(\lambda_j, T)\right) = \sum_{j=1}^{m} \dim E(\lambda_j, T) \right)$.
  ...
  Choose a basis of each $E(\lambda_k, T)$; put all these together to form a list
  \begin{equation}
  ...
  denote the $k$-th sum of the terms $a_jv_j$, where the $v_j$'s belong to the basis of $E(\lambda_k, T)$. This means that $u_k \in E(\lambda_k, T)$.
...
\end{thm}
\begin{prf}
  Let $v_1, \ldots, v_{\dim V}$ be eigenvectors corresponding to the $\dim V$ distinct eigenvalues $\lambda_1, \ldots, \lambda_{\dim V}$; they are linearly independent by \ref{thm: linearly independent eigenvectors}. So, we have a basis consisting of $\dim V$ eigenvectors. Hence, $T$ is diagonalizable by \ref{thm: conditions equivalent to diagonalizability}.
\end{prf}
...
\end{thm}
\begin{prf} For this proof, let $T\in \linmap (V)$, where $V$ is finite-dimensional ($\dim V \neq \infty$).
  ...
  which shows that $(T-\lambda_1 I) \cdots (T-\lambda_m I)$ is $0$ on a basis of $V$, and hence is the zero operator. So $(z-\lambda_1)\cdots(z-\lambda_m)$ is a polynomial multiple of the minimal polynomial of $T$ by \ref{thm: every zero polynomial is a multiple of the minimal polynomial}. Conversely, each $\lambda_k$ is an eigenvalue of $T$ and is therefore a zero of the minimal polynomial by \ref{thm: eigenvalues are the zeros of the minimal polynomial}, so each factor $(z-\lambda_k)$ divides it. Because the $\lambda_k$ are distinct, the minimal polynomial equals $(z-\lambda_1)\cdots(z-\lambda_m)$.
...
%  \end{equation}
  ...
  By our induction hypothesis, there is a basis of $U$ consisting of eigenvectors of $\left . T \right |_U$, which are therefore also eigenvectors of $T$.
  Suppose
  \begin{equation}
...
\begin{prf}
  ...
  Hence, the minimal polynomial of $\left.T\right|_U$ has the form required by \ref{thm: necessary and sufficient condition for diagonalizability}, which shows that $\left.T\right|_U$ is diagonalizable: its factors are among $(z-\lambda_1), \ddd, (z-\lambda_m)$, and they are distinct.
\end{prf}
