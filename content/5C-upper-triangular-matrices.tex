...
\begin{prf}
  ...
  Altogether, we have shown that (a) $\implies$ (b) $\implies$ (c) $\implies$ (a).
\end{prf}
...
\end{aligned}
  \end{equation}
  (Here we use that any two polynomials in $T$ commute; see \ref{multiplicative-properties}.)
  \bigbreak
...
\begin{prf}
  ...
  this implies that the restriction of $T-\lambda_k I$ to $\myspan{v_1, \ldots, v_k}$ is not injective by \ref{thm: linear-map-to-a-lower-dimensional-space-is-not-injective}. This theorem states that a linear map to a lower-dimensional space is not injective. Thus,
  \begin{equation}
    \exists v \in \myspan{v_1, \ldots, v_k}: v\neq 0 \text{ and } (T-\lambda_k I)v=0.
  \end{equation}
...
\begin{prf}
  ...
  because of \eqref{eq: definition of the minimal polynomial}. Hence, the polynomial $(z-\lambda_1) \cdots (z-\lambda_{m-1})$ is a polynomial multiple of the minimal polynomial of $\left. T \right |_{U}$, by \ref{thm: every zero polynomial is a multiple of the minimal polynomial}. In other words, the minimal polynomial of $\left. T \right |_{U}$ is a product of at most $m-1$ terms of the form $(z-\lambda_k)$ for $k = 1, \ddd, m-1$.
