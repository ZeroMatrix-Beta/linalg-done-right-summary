...
\begin{prf}
  ...
  \StepThree Finally, suppose (c) holds. Hence, $T v_k \in \myspan{v_1, \ldots, v_k}$ for $k = 1, \ddd, n$. This implies that there exist $a_k, \dots, a_n \in \myF$, such that
  \begin{equation}
    \label{eq: equation for T v_k}
    T v_k = a_1 v_1 + \cdots + a_k v_k = a_1 v_1 + \cdots + a_k v_k + 0 v_{k+1} + \cdots + 0 v_n.
  \end{equation}

  Hence, all the entries below the diagonal of $\mmatrix (T)$ are $0$, because the list $v_1, \ldots, v_n$ is linearly independent and we cannot choose another representation for $T v_k$ in respect to that list. Thus, $\mmatrix(T)$ is an upper-triangular matrix, completing the proof that (c) implies (a).
...
\begin{prf}
  ...
  Thus, $\mmatrix(T)$ is upper triangular. Therefore we are ready to use induction over $m$.
