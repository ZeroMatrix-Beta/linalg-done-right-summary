...
% 2.34
\setcounter{thm}{33}
\begin{thm} [basis length] Any two bases of a \fdvs have the same length.
  \label{thm: basis length}
\end{thm}
\begin{prf}
  Let $B_1$ and $B_2$ be two bases of a finite-dimensional vector space $V$.
  Since $B_1$ is linearly independent and $B_2$ spans $V$, \ref{thm: length of linearly dependent list less or equal to length of spanning list} implies $|B_1| \leq |B_2|$.
...
\begin{prf}
  ...
  By \ref{thm: length of linearly dependent list less or equal to length of spanning list}, the length of any linearly independent list in $V$ is less than or equal to the length of any spanning list in $V$. Therefore, $m \leq n$, which means $\dim U \leq \dim V$.
\end{prf}
...
\begin{prf}
  ...
  However, every basis of $V$ has length $n$, so no vectors can be removed during the reduction process.
...
\begin{equation}
  ...
  r_1, \ddd, r_n, u_1, \ddd, u_j, w_1, \ddd, w_k
\end{equation}
is a basis of $V_1 + V_2$. Once this is established, the proof is complete because the length of this basis is
\begin{equation}
  \begin{aligned}
    \dim (V_1 + V_2) &= n + j + k \\
    &= (n+j) + (n+k) - n \\
...
Clearly, the list \eqref{eq: basis with r u and w} is contained in $V_1 + V_2$, and its span contains both $V_1 = \myspan{r_1, \dots, r_n, u_1, \dots, u_j}$ and $V_2 = \myspan{r_1, \dots, r_n, w_1, \dots, w_k}$. Thus, it spans $V_1 + V_2$.

It remains to prove linear independence. Suppose there exist scalars $a_1, \ddd, a_n, b_1, \ddd, b_j, c_1, \ddd, c_k \in \myF$ such that
\begin{equation}
