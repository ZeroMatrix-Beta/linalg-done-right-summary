...
\subsection{Invertible Linear Maps}
  ...
  \end{equation}
    ...
    The column $\mmatrix(u)$ depends on the basis, even though the basis is not included in the notation.
%    Recall:
%\begin{equation}
%    \mathcal{M} (T) :\equiv
...
%    $\mmatrix_{\mathsmaller{\bullet}, k} = M(T v_k)$. The $k^{\text{th}}$ column of $\mmatrix_{\mathsmaller{\bullet}, k}$ equals $(A_{1,k}, \dots, A_{m,k})^\top$
  ...
  \begin{thm} [matrix of product of linear maps]
    ...
    Suppose $T\in \mathcal{L}(U,V)$ and $S\in \lvw$, where $\onetillm{u}$ is a basis of $U$, $\onetilln{v}$ is a basis of $V$, and $\onetill{w}{p}$ is a basis of $W$, so that $\dim U = m$, $\dim V = n$, and $\dim W = p$. Then we have:
    \begin{equation}
    \begin{aligned}
  ...
  \begin{prf}
    ...
    \begin{equation}
      \m{I_V} = I_m = \m{I_V I_V} = \m{I_V, (v_1, \ddd, v_m), (u_1, \ddd, u_m)} \cdot \m{I_V, (u_1, \ddd, u_m), (v_1, \ddd, v_m)}.
    \end{equation}

    Now, interchange the roles of the $u$'s and the $v$'s, getting
    \begin{equation}
      \m{I_V} = I_m = \m{I_V I_V} = \m{I_V, (u_1, \ddd, u_m), (v_1, \ddd, v_m)} \cdot \m{I_V, (v_1, \ddd, v_m), (u_1, \ddd, u_m)}.
    \end{equation}

    So, the product of both matrices equals $I_m$, which is the identity in $\myF^{m,m}$.
  \end{prf}
  ...
  \setcounter{thm}{85}
  \begin{thm}
    If $v_1, \ddd, v_m$ is a basis of $V$ and $T \in \linmap(V)$ is invertible, then
    \begin{equation}
