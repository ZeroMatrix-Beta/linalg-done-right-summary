...
\begin{prf}
  ...
  \StepOne First, suppose (a) holds, so $S$ is an isometry. If $v\in V$, then:
  \[
  ...
  \StepTwo Now, suppose (b) holds, so $S^*S=I$. If $u,v \in V$, then:
  \[
  \ip{Su}{Sv} = \ip{S^*Su}{v} = \ip{Iu}{v} = \ip{u}{v},
  \]
  proving $(b) \implies (c)$.

  \StepThree Now, suppose that (c) holds, so $\ip{Su}{Sv} = \ip{u}{v} \quad \forall u,v \in V$. Thus, if $j,k \in \{1, \ddd, n\}$, then:
  \[
  ...
  \StepFour Suppose (d) holds. The $k$-th column of $\mmatrix(S) \equiv \mmatrix(S, (e_1, \ddd, e_n), (f_1, \ddd, f_m))$ consists of the scalars $(\ip{Se_k}{f_1}, \ddd, \ip{Se_k}{f_m})^t$. By \ref{thm: writing a vector as a linear combination of an orthonormal basis}(c), the Euclidean inner product in $\myF^m$ of the $j$-th and $k$-th columns of $\mmatrix(S)$ equals
  \[
  ...
  \StepFive Suppose (e) holds. Then the vectors $Se_1, \ddd, Se_n$ form an orthonormal list in $W$. For any $v \in V$, write $v = a_1 e_1 + \cdots + a_n e_n$ for some $a_1, \ddd, a_n \in \myF$. Then $Sv = a_1 Se_1 + \cdots + a_n Se_n$. By \ref{thm: norm of an orthonormal linear combination}:
  \[
