...
\begin{thm}[each zero of a polynomial corresponds to a degree\-/one factor]
  ...
  \begin{equation}
  	p(\lambda) = 0 \iff \exists q \in \mathcal{P}_{m-1}: p(z) = (z-\lambda)q(z), \quad \forall z \in \myF.
  \end{equation}
\end{thm}
...
\begin{prf}
  ...
  Therefore, if $b^2<4c$, the first and the second term are always positive and by \ref{factororing-out-zeros-of-a-polynomial-always-possible}, no factoriza\-tion is possible, because $p$ has no zeros.
...
\begin{prf}
  ...
  Since according to \ref{thm: a polynomial of degree m has at most m zeros} a complex polynomial with degree $n-2$ can have at most $n-2$ zeros, this equation cannot hold for all $x \in \real$. This implies that $\impart a_0, \ddd, \impart a_{n-2}$ all equal $0$, because only the zero polynomial is allowed to have infinitely many zeros. Thus, all the coefficients of $q$ are real, as desired. Hence, the wanted factorization exists by induction.
