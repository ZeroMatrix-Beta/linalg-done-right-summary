...
\begin{xprf}
  ...
  Since $T^* e_j = \overline{\lambda_j} e_j$, we have:
  \[
    T = T^* \iff T e_j = T^* e_j \;\forall j \iff \lambda_j e_j = \overline{\lambda_j} e_j \;\forall j \iff \lambda_j = \overline{\lambda_j} \;\forall j \iff \lambda_j \in \real \;\forall j.
  \]
\end{xprf}
\end{xrcs}
...
\exercise{4}
\begin{xrcs}
  Prove that a normal operator on a complex inner product space is skew (meaning $T^* = -T$) if and only if all its eigenvalues are purely imaginary (meaning $\realpart \lambda = 0$).
...
\begin{xprf}
  ...
  Then $T^* e_j = \overline{\lambda_j} e_j$.
  Therefore:
  \[
    T^* = -T \iff T^* e_j = -T e_j \;\forall j \iff \overline{\lambda_j} e_j = -\lambda_j e_j \;\forall j \iff \overline{\lambda_j} = -\lambda_j \;\forall j \iff \realpart \lambda_j = 0 \;\forall j.
  \]
\end{xprf}
\end{xrcs}
