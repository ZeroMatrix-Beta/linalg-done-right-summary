...
\begin{xrcs}
  ...
  \begin{xprf}
    ...
    \begin{failedattempt}[Earlier draft (never finished)]
    ...
    \begin{ainote}[What this draft got right, and two traps it avoided by stopping]
      This one is incomplete rather than wrong. The setup is exactly the setup of the finished proof --- split off $\mynull S$, work on a complement, use $\myrange S \subseteq \myrange T$ to obtain preimages --- and it stops at \qt{define $E$ on the basis, and verify}, which is where the actual work begins. Two things have to be got right at that point, and both are easy to get wrong:

      \emph{Do not try to route the proof through null spaces.} A natural-looking move is to claim $\myrange S \subseteq \myrange T \implies \mynull T \subseteq \mynull S$. That implication is false. On $V = W = \myF^2$ take
      \begin{equation}
        S(x,y) := (x,0) \myand T(x,y) := (y,0).
      \end{equation}
      Then $\myrange S = \myrange T = \myspan{(1,0)}$, so the hypothesis holds, yet $\mynull T = \myspan{(1,0)}$ while $\mynull S = \myspan{(0,1)}$, and neither contains the other. Range containment simply carries no information about null spaces in this direction.

      \emph{Do not identify $S$ and $T$ on the complement.} It is tempting to write $S|_U = T|_U$ once preimages have been chosen, but $S$ and $T$ are unrelated maps; the hypothesis only says their \emph{ranges} are nested. The preimage $p_j$ with $T p_j = S q_j$ generally has nothing to do with $q_j$, and that is precisely why $E$ must be built to send $q_j \mapsto p_j$ rather than being read off from $S$ and $T$ directly.

      The finished proof above does exactly that, and it is worth noticing that the choice of $E$ is not unique: any preimage $p_j$ will do, and the vectors $r_k$ spanning $\mynull S$ may be sent anywhere in $\mynull T$, not only to $0$.
    \end{ainote}
  \end{xprf}
\end{xrcs}
