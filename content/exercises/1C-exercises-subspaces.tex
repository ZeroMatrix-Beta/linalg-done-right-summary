  ...
  \begin{enumerate}
    \item {
      ...
      \begin{xsol}
        ...
        Only if $b=0$, it can hold that the third coordinate of $v+u$ is $5$ times the fourth coordinate of $v+u$ plus an element of $\myF$ acting as $b$. %, given the constraints of $v$ and $u$ themselves.
    ...
    \item{
      The set of continuous real-valued functions on the interval $[0,1]$ is a subspace of $\real^{[0,1]}$ (The set of real valued functions on the interval $[0,1]$)

      \begin{xsol}
        The constant function $0$ is continuous. Adding two continuous functions together yields a continuous function. Multiplying a continuous function with a scalar also gives us a continuous function.
      \end{xsol}
    }
    ...
    \item{
      ...
      \begin{xsol}
        ...
        \emph{Closedness under addition:} Let $h(x) := f(x) + g(x)$ for two differentiable real-valued functions $f$ and $g$. Then $h'(x) = \frac{d}{dx} (f(x)+g(x)) = f'(x) + g'(x)$, so this sum $h$ also belongs to set of differentiable real-valued functions.

        \emph{Closedness under multiplication:} Let $h(x) := c f(x)$. Then $h'(x) = \frac{d}{dx} (c f(x)) = c f'(x)$. So this product $h$ also belongs to the set of differentiable real-valued functions.
      \end{xsol}
    }
...
\begin{xrcs}
  ...
  \begin{xsol}
    This statement is wrong. Take $U :\equiv \{(x,y) \in \real^2 \mid (x,y) \in \integer^2 \} \subset \real^2$. Then $U$ is closed under addition since $\integer$ is closed under addition. Example: $(2,1)+(-2,1)+(1,1) = (1,1) \in U$. But if $\lambda \in \real$ such that $\lambda \notin \integer$ we have for $a,b \in \integer$:
  \[
