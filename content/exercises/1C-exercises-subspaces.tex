...
%   By definition, $V_1 + \dots + V_m$ is a direct sum if and only if this concatenated list is linearly independent in $V$.
%   A spanning list is linearly independent if and only if its length equals the dimension of its span, which holds if and only if
