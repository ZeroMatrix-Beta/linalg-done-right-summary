...
\begin{xprf}
  \Rightarrowdirection If $T = 0$, then $T^*T = 0$, whose only eigenvalue is $0$ (with multiplicity $\dim V$). Thus all singular values of $T$ are $\sqrt{0} = 0$.
...
\begin{xprf}
  \Rightarrowdirection Suppose $s$ is a singular value of $T$. By the Singular Value Decomposition (\ref{thm: singular value decomposition}), there exist orthonormal lists $e_1, \dots, e_m$ in $V$ and $f_1, \dots, f_m$ in $W$ such that $T e_j = s_j f_j$ and $T^* f_j = s_j e_j$.
  Since $s$ is a singular value, $s = s_k$ for some $k$. Setting $v = e_k \neq 0$ and $w = f_k \neq 0$ yields $Tv = sw$ and $T^*w = sv$.

  \Leftarrowdirection Conversely, suppose nonzero vectors $v \in V, w \in W$ satisfy $Tv = sw$ and $T^*w = sv$.
  Then $T^*Tv = T^*(sw) = s T^*w = s(sv) = s^2 v$.
  Since $v \neq 0$, $s^2$ is an eigenvalue of $T^*T$.
...
\begin{xprf}
  Since $T$ is self-adjoint ($T^* = T$), we have $T^*T = T^2$.
  ...
  Then:
  \[
    T^* T e_j = T^2 e_j = T(\lambda_j e_j) = \lambda_j^2 e_j.
  \]
  Thus the eigenvalues of $T^*T$ are $\lambda_1^2, \dots, \lambda_n^2$.
  The singular values of $T$ are the non-negative square roots of the eigenvalues of $T^*T$, which are $\sqrt{\lambda_j^2} = |\lambda_j|$ for $j = 1, \dots, n$.
