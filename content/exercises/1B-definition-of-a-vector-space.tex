...
\begin{xrcs}
  ...
  \begin{xprf}
    Let $v \in V$. By definition, we know that $v + (-v) = 0$. We also know by definition, that the additive inverse of $(-v)$, which is unique by \ref{thm: unique additive identity}, is denoted by $-(-v)$. The very first equations shows that $-(-v) = v$, since $v + (-v) = -(-v) + (-v) = 0$.
  \end{xprf}
\end{xrcs}
...
\begin{xrcs}
  \textbf{KI Bemerkung: Hier fehlt die eigentliche Fragestellung (z. B. "Show that the empty set is not a vector space.").}
  
  Show that the empty set is not a vector space.

  \begin{xsol}
    The only requirement for a vector space the empty set fails to satisfy is the existence of an additive identity.
  \end{xsol}
\end{xrcs}
