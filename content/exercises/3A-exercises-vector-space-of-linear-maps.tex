...
%  \begin{xprf}
%    If $T$ is the zero operator, then the statement holds trivially for $\lambda = 0$. Therefore, assume $T \neq 0$. If $\mydim V = 1$, every $v \in V$ can be written as $a_1 v_1$, where $a_1 \in \myF$ and $v_1 \in V$. In this case, $v_1$ would denote the base vector of a one dimensional base for $V$. If we apply $T$, we get $Tv = T(a_1 v_1) = a_1 T v_1$. If we define $w_1 := T v_1$, this makes $w_1$ a base vector of $\myrange T$ and we have $Tv = a_1 w_1$. Since $V$ is one dimensional, we can express $w_1$ in terms of $v_1$, i.e. $w_1 = \lambda v_1$ for some $\lambda \in \myF$. Thus, we have
%    \begin{equation}
