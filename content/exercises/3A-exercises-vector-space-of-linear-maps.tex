...
\begin{xrcs}
  ...
  \begin{xprf}
    Both of the vectors we need are available. Since $S \neq 0$, we may \emph{choose} $u \in U$ with $Su \neq 0$, and since $U \neq V$ we may choose $v \in V$ with $v \notin U$ (i.e. $v \in V \backslash U$). Fix these two for the rest of the proof.

    Consider the sum $u + v$. We claim that $u+v \notin U$, because if $u+v \in U$ then by closure of $U$, we would have $(u+v)-u = v \in U$, which contradicts $v \notin U$. Therefore, it holds that
    \begin{equation}
      T(\underbrace{u+v}_{\notin \; U}) = 0.
    \end{equation}

    Whereas, since $u \in U$ and $v \notin U$,
    \begin{equation}
      Tu + Tv = Su + 0 = Su.
    \end{equation}

    By the choice of $u$ we have $Su \neq 0$, so $T(u+v) = 0 \neq Su = Tu + Tv$. This demonstrates that $T$ is not a linear map because it fails to satisfy additivity. The failure of $T$ to be a linear map arises because $T$
     is defined differently on $U$ and $V \backslash U$, breaking the additivity required for linearity.
  \end{xprf}
\end{xrcs}

\begin{ainote}[Choosing $u$ is not optional]
  It matters that $u$ is \emph{chosen} with $Su \neq 0$ rather than taken arbitrarily. For an arbitrary $u \in U$ the computation still yields $T(u+v) = 0$ and $Tu + Tv = Su$, but if that particular $u$ happens to satisfy $Su = 0$ the two sides agree and nothing is contradicted. Additivity fails as soon as it fails for \emph{one} pair of vectors, so a single well-chosen witness is exactly what the proof needs --- and the hypothesis $S \neq 0$ is what guarantees one exists.

  The same remark explains why both hypotheses are needed. Drop $S \neq 0$ and $T$ becomes the zero map, which is linear. Drop $U \neq V$ and there is no $v$ outside $U$, so $T = S$, which is linear as well. Neither hypothesis is decoration.
\end{ainote}
