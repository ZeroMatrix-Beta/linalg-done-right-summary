...
\begin{xrcs}
  ...
  \begin{xsol}
    ...
    Hence, beeing and eigenvalue $\lambda$ and an eigenvector $\varphi \in \dual{V} = \linmap(V, \myF)$ of $\dual{T}$ means \begin{equation}
    ...
    we have found and eigenvector $\psi$ with the same eigenvalue $\lambda$.
  \end{xsol}
\end{xrcs}
...
\begin{xrcs}
  ...
  \begin{xprf}
    ...
    Then we can choose a subspace $U$ of with $V = U \oplus \myspan{v_0}$. We already know that $\myspan{v}$ is invarant under $T$. Let $u_1, \ddd, u_{m}$ be a basis of $U$ with $m = \mydim V - 1$. Hence,
  \end{xprf}
...
\begin{xrcs}
  ...
  \begin{xprf}
    ...
    \begin{enumerate}
      ...
      \item{
        \Rightarrowdirection Suppose $T$ is surjective. For $m=1$ the statement is trivial. Fix $m \geq 1$ and suppose as induction hypothesis that $T^m$ is surjective. Let $y\in V$ be arbitrary. Since $T$ is surjective there exists $z\in V$ with $T(z)=y$. By the induction hypothesis, there exists $v\in V$ with $T^m(v)=z$. Thus,
        \begin{equation}
          T^{m+1}(v)=T(T^m(v))=T(z)=y.
        \end{equation}

        Hence, $T^{m+1}$ is surjective. (Equivalently: a finite composition of surjective maps is surjective.)

        \Leftarrowdirection Assume $T^m$ is surjective for some fixed $m\ge 1$. Let $y\in V$ be arbitrary. Choose $v\in V$ with $T^m(v)=y$ (possible since $T^m$ is surjective). Put $x:=T^{m-1}(v)$. Then
        \begin{equation}
