...
\begin{mydef} [invariant subspace]
  Let $T\in \linmap(V).$ A subspace $U$ of $V$ is called \qt{invariant under $T$} if $\forall u \in U: Tu \in U.$ \\
...
\end{thm}
\begin{prf}
  Suppose the desired result is false. Then there exists a smallest list of length $m$ of linearly depen\-dent eigenvectors $v_1, \dots, v_m$ with distinct eigenvalues $\lambda_1,$ $\dots$ $,\lambda_m$. Since an eigenvector is unequal to the zero vector, $m$ must be $\geq 2$.
