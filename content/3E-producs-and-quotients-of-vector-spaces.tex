...
\begin{prf}
  ...
  \underbrace{
    \left (
    \begin{matrix}
      v_{1_1} \\ 0 \vphantom{\vdots} \\ \vdots \\ \vdots \\ \vdots \\ 0 \vphantom{\vdots}
    \end{matrix}
    \right )
    \; \mathlarger{,} \;
    \left (
    \begin{matrix}
      v_{1_2} \\ 0 \vphantom{\vdots} \\ \vdots \\ \vdots \\ \vdots \\ 0 \vphantom{\vdots}
    \end{matrix}
    \right )
    \mathlarger{,}
    \mathlarger{\ddd}
    ...
    \left (
    \begin{matrix}
      v_{1_{\dim V_1}} \\ 0 \vphantom{\vdots} \\ \vdots \\ \vdots \\ \vdots \\ 0 \vphantom{\vdots}
    \end{matrix}
    \right )
  }_{\text{constructed from basis vectors of $V_1$}}
  \;
  \mathlarger{,}
  \mathlarger{\ddd}
  \mathlarger{,}
  ...
  \underbrace{
    \left (
    \begin{matrix}
      0 \vphantom{\vdots} \\ \vdots \\ v_{k_1} \\ 0 \vphantom{\vdots} \\ \vdots \\ 0 \vphantom{\vdots}
    \end{matrix}
    \right )
    \; \mathlarger{,} \;
    \left (
    \begin{matrix}
      0 \vphantom{\vdots} \\ \vdots \\ v_{k_{2}} \\ 0 \vphantom{\vdots} \\ \vdots \\ 0 \vphantom{\vdots}
    \end{matrix}
    \right )
    \mathlarger{,}
    \mathlarger{\ddd}
    ...
    \left (
    \begin{matrix}
      0 \vphantom{\vdots} \\ \vdots \\ v_{k_{\dim V_k}} \\ 0 \vphantom{\vdots} \\ \vdots \\ 0 \vphantom{\vdots}
    \end{matrix}
    \right )
  }_{\text{constructed from basis vectors of $V_k$}}
  \mathlarger{,}
  \mathlarger{\ddd}
  \mathlarger{,}
  \underbrace{
      \left (
      \begin{matrix}
        0 \vphantom{\vdots} \\ \vdots \\ \vdots \\ \vdots \\ 0 \vphantom{\vdots} \\ v_{m_1}
      \end{matrix}
      \right )
      \; \mathlarger{,} \;
      \left (
      \begin{matrix}
        0 \vphantom{\vdots} \\ \vdots \\ \vdots \\ \vdots \\ 0 \vphantom{\vdots} \\ v_{m_2}
      \end{matrix}
      \right )
      \mathlarger{,}
      \mathlarger{\ddd}
      ...
      \left (
      \begin{matrix}
        0 \vphantom{\vdots} \\ \vdots \\ \vdots \\ \vdots \\ 0 \vphantom{\vdots} \\ v_{m_{\dim V_m}}
      \end{matrix}
      \right )
  }_{\text{constructed from basis vectors of $V_m$}}
  \]

  The list of all such vectors is linearly independent and spans $V_1 \times \cdots \times V_m$. Thus, it is a basis of $V_1 \times \cdots \times V_m$, and the length of this basis is $(\dim V_1 + \cdots + \dim V_m)$.
\end{prf}

In the next result, the map $\Gamma$ is surjective by the definition of $V_1 + \cdots + V_m$: every element of that sum is, by definition, of the form $v_1 + \cdots + v_m$ with each $v_k \in V_k$. Thus, the last word in the result below could be changed from \qt{injective} to \qt{invertible}.
...
\end{thm}
\begin{prf}
  By \ref{thm: injectivity iff null space equals zero-set}, $\Gamma$ is injective $\iff \mynull \Gamma = \{ \vec 0 \}$. Now $\mynull \Gamma$ consists of those $(v_1, \ddd, v_m)$ with $v_1 + \cdots + v_m = 0$, so $\mynull \Gamma = \{ \vec 0 \}$ says precisely that the only way to write $0$ as a sum $v_1 + \cdots + v_m$, where each $v_k \in V_k$, is by taking each $v_k=0$. Thus, \ref{thm: condition for a direct sum} shows that $\Gamma$ is injective $\iff V_1 + \cdots + V_m$ is a direct sum, as desired.
\end{prf}
...
\begin{thm}[a sum is a direct sum if and only if the dimensions add up]
  ...
  \begin{equation}
    V_1 + \cdots + V_m \text{ is a direct sum (i.e., } \sum_{i=1}^m V_i = \bigoplus_{i=1}^m V_i \text{)} \iff
    \dim (V_1+\cdots+V_m) = \dim V_1 + \cdots + \dim V_m.
  \end{equation}
\end{thm}

\begin{prf}
  The map $\Gamma: V_1 \times \cdots \times V_m \to V_1 + \cdots + V_m$ in \ref{thm: products and direct sums} is surjective, so $\myrange \Gamma = V_1 + \cdots + V_m$. Thus, the rank-nullity theorem \ref{rank-nullity-theorem} gives
  \[
    \dim (\underbrace{V_1 \times \cdots \times V_m}_{\text{domain}}) = \dim \mynull \Gamma +  \dim(\underbrace{V_1 + \cdots + V_m}_{\text{codomain} \; \subseteq \; V}).
  \]

  We can use \ref{thm: dimension of a product is the sum of dimensions} to conclude that the left-hand side equals $\dim V_1 + \cdots + \dim V_m$, so
  \begin{equation}
    \dim V_1 + \cdots + \dim V_m = \dim \mynull \Gamma + \dim (V_1 + \cdots + V_m).
  \end{equation}

  Reading this equation as a statement about $\dim \mynull \Gamma$ gives a chain of equivalences:
  \begin{equation}
    \begin{aligned}
      \dim (V_1 + \cdots + V_m) = \dim V_1 + \cdots + \dim V_m
      & \iff \dim \mynull \Gamma = 0 \\
      & \iff \Gamma \text{ is injective} \\
      & \iff V_1 + \cdots + V_m \text{ is a direct sum,}
    \end{aligned}
  \end{equation}

  where the second equivalence holds by \ref{thm: injectivity iff null space equals zero-set} and the third by \ref{thm: products and direct sums}. This is exactly the desired result.

  \prooffont{Special case for m=2:} For the special case where $m=2$, we can use \ref{thm: dimension of a sum of subspaces}, where
  \[
    \dim (V_1 + V_2) = \dim V_1 + \dim V_2 - \dim (V_1 \cap V_2),
  \]

  and we get $\dim (V_1 \cap V_2) = 0 \iff V_1 + V_2 = V_1 \oplus V_2$ by \ref{thm: sum and intersection of two subspaces}.
\end{prf}
...
\begin{prf}
  ...
  \[
    v+u = (w - w + v + u) = w + \Big( \underbrace{(v-w)}_{\in \; U} + u\Big), \text{ thus } v+u \in w+U.
  \]

  Similarly, because $v-w \in U$, we also have $w-v \in U$, and thus,
  \[
    w+u = (v - v + w + u) = v+\Big( \underbrace{(w-v)}_{\in \; U}+u \Big), \text{ hence } w+u \in v+U.
  \]
...
\begin{thm}[dimension of quotient space]
  ...
  Suppose $\dim V < \infty$ and $U \subseteq V$ is a subspace. Then
  \begin{equation}
    \dim V/U = \dim V - \dim U.
  \end{equation}
\end{thm}
\begin{prf}
  Let $\pi: V \to V/U$ be the quotient map $\pi(v) = v + U$.
  The map $\pi$ is surjective, so $\myrange \pi = V/U$. Furthermore, $v \in \mynull \pi \iff v + U = 0 + U \iff v \in U$, so $\mynull \pi = U$.
  Applying the Rank-Nullity Theorem (\ref{rank-nullity-theorem}) to $\pi$ gives
  \begin{equation}
    \dim V = \dim \mynull \pi + \dim \myrange \pi = \dim U + \dim V/U.
  \end{equation}
  Subtracting $\dim U$ from both sides yields $\dim V/U = \dim V - \dim U$.
\end{prf}
...
\begin{thm}[null space and range of $\widetilde{T}$]
  \label{thm: null space and range of T tilde}
  Suppose $T\in \linmap(V,W)$. Then:
  \begin{enumerate}[label=\textbf{(\alph*)}]
    \item $\widetilde{T} \circ \pi = T,$ where $\pi$ is the quotient map of $V$ onto $V/(\mynull T)$;
...
\begin{prf}
  \begin{enumerate}[label=\textbf{(\alph*)}]
    \item For every $v \in V$, $(\widetilde{T} \circ \pi)(v) = \widetilde{T}(\pi(v)) = \widetilde{T}(v + \mynull T) = Tv$, so $\widetilde{T} \circ \pi = T$.
    \item Suppose $v + \mynull T \in \mynull \widetilde{T}$. Then $0 = \widetilde{T}(v + \mynull T) = Tv$, so $v \in \mynull T$, which means $v + \mynull T = 0 + \mynull T$. Thus $\mynull \widetilde{T} = \{0 + \mynull T\}$, proving that $\widetilde{T}$ is injective.
    \item $\myrange \widetilde{T} = \{\widetilde{T}(v + \mynull T) \mid v \in V\} = \{Tv \mid v \in V\} = \myrange T$.
    \item Since $\widetilde{T}$ is a linear map that is injective and maps onto $\myrange T$, it is an isomorphism from $V/(\mynull T)$ onto $\myrange T$.
  \end{enumerate}
  \vspace{-\baselineskip}
\end{prf}