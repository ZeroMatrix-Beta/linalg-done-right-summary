...
\begin{prf}
  \begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \item Let $u = \sum_{j=1}^m \ip{v}{e_j} e_j$. In \ref{thm: Bessel's inequality}, we showed that $u \in U$ and $v - u \in U^\bot$. By definition of $P_U$, $P_U v = u$.
  \end{enumerate}
  \vspace{-\baselineskip}
\end{prf}
...
\begin{prf}
  ...
  We also have the properties that $v-P_Uv \in U^\bot$ and $P_U v - u \in U$. Therefore, we can use the Pythagorean theorem \ref{thm: pythagorean theorem} after adding an always-positive term to the left-hand side of the inequality:
  \[
  \begin{aligned}
    \norm{v-P_Uv}^2
...
\begin{prf}
  ...
  To prove that $\myrange \left.T \right | _{(\mynull T)^\bot}$ is onto $\myrange T$, we will show that $\myrange \left. T \right | _{(\mynull T)^\bot} \subseteq \myrange T$ and then $\myrange T \subseteq \myrange \left. T \right | _{(\mynull T)^\bot}$. Because this will imply that $\myrange T = \myrange \left. T \right |_{(\mynull T)^\bot}$ and therefore surjectivity.
