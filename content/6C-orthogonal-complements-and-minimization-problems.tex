...
\begin{prf}
  ...
  \begin{enumerate}[label=\textbf{(\alph*)}]
    ...
    \item Suppose $G\subseteq V$ and $G \subseteq V$ and $G \subseteq H$. [As usual, we misuse $\subseteq$ to denote subspaces.]
    \[
      \begin{aligned}
        \text{Let $v\in H^{\bot}$.}
...
\begin{prf}
  ...
  \[
    \forall k \in \{1, \ddd, m\}: e_k \in U. \implies u \in U, \quad $because it is a linear combination of $e_1, \ddd, e_m.
  \]
...
\begin{prf}
  ...
  Suppose $v \in (\mynull T)^\bot$ \st $\left. T \right | _{(\mynull T)^\bot}=0$. Hence $Tv=0$ and $v \in (\mynull T) \cap (\mynull T)^\bot$, which implies that $v=0$ by \ref{thm: properties of the orthogonal complement}, because $(\mynull T) \cap (\mynull T)^\bot \subseteq \{0\}$.
  \[
